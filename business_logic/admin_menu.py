...
def callback_inline_admin(call: CallbackQuery):
    if call.message:
        if call.data == 'Личный кабинет':
            ...
            mark_up = types.InlineKeyboardMarkup(row_width=2)
            item1 = types.InlineKeyboardButton('Связь с пользователями', callback_data='Связь с пользователями')
            item2 = types.InlineKeyboardButton('Цены на тарифы', callback_data='Цены на тарифы')
            item3 = types.InlineKeyboardButton('Управление балансом', callback_data='Управление балансом пользователя')
            item4 = types.InlineKeyboardButton('Просмотр пользователя', callback_data='Просмотр пользователя')
            item5 = types.InlineKeyboardButton('Рассылка пользователям', callback_data='Рассылка пользователям')
            item6 = types.InlineKeyboardButton('Статистика', callback_data='Статистика')
            item7 = types.InlineKeyboardButton('Каналы', callback_data='Каналы')
            item8 = types.InlineKeyboardButton('Назад к главной панели', callback_data='Назад админ')
            mark_up.add(item1, item2, item3, item4, item5, item6, item7, item8)
            bot.send_message(call.message.chat.id, 'Выберите действие, {0.first_name}'.format(call.from_user),
                             reply_markup=mark_up)

        elif call.data == 'Ответ техподдержки':
            mark_up = types.InlineKeyboardMarkup(row_width=2)
            back = types.InlineKeyboardButton('Назад', callback_data='Назад админ')
            mark_up.add(back)
            bot.send_message(call.message.chat.id, 'Чуть позже появится', reply_markup=mark_up)

        elif call.data == 'Цены на тарифы':
            mark_up = types.InlineKeyboardMarkup(row_width=2)
            partial = types.InlineKeyboardButton('Частичный', callback_data='Частичный')
            average = types.InlineKeyboardButton('Средний', callback_data='Средний')
            full = types.InlineKeyboardButton('Полный', callback_data='Полный')
